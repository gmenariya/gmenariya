...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.testmodule.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.testmodules.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group {{ $errors->has('radiox') ? 'has-error' : '' }}">
                ...
                @foreach(App\Testmodule::RADIOX_RADIO as $key => $label)
                    <div>
                        <input id="radiox_{{ $key }}" name="radiox" type="radio" value="{{ $key }}" {{ old('radiox', null) === (string)$key ? 'checked' : '' }}>
            ...
            <div class="form-group {{ $errors->has('selectx') ? 'has-error' : '' }}">
                ...
                <select id="selectx" name="selectx" class="form-control">
                    ...
                    @foreach(App\Testmodule::SELECTX_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('selectx', null) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('checkboxx') ? 'has-error' : '' }}">
                ...
                <input value="1" type="checkbox" id="checkboxx" name="checkboxx" {{ old('checkboxx', 0) == 1 || old('checkboxx') === null ? 'checked' : '' }}>
